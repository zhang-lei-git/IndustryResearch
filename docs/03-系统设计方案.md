...
| SamplingStrategy | id、taskId、rules、priorityRules、createdAt、status | 企业选样规则及推荐依据 |
| ResearchSample | id、taskId、companyId、sampleRole、selectionReason、priority、status、snapshotAt | 任务样本及选择理由；不等同企业主数据 |
| QuestionSet | id、taskId、companyId、name、focus、version、status、generatedAt、frozenAt | 面向一次调研重点的问题组 |
...
`候选样本 → 已选定 → 已计划 → 已预约 → 待执行 → 执行中 → 已完成/需复访/已取消`。计划对象进入已完成时必须存在唯一调研记录；同一企业的再次调研通过新计划对象承载。
