...
| ResearchTask | id、workspaceId、name、objective、topicIds、ownerId、startAt、endAt、status | 一轮调研的业务边界 |
| SamplingStrategy | id、taskId、rules、priorityRules、createdAt、status | 可保存的企业库筛选规则；筛选本身不自动创建调研对象 |
| ResearchSample | id、taskId、companyId、sampleRole、selectionReason、priority、status、snapshotAt | 用户从企业库筛选结果明确加入的调研对象；不等同企业主数据 |
| QuestionSet | id、taskId、companyId、name、focus、version、status、generatedAt、frozenAt | 绑定项目和调研对象的访谈提纲 |
| QuestionSetItem | id、questionSetId、category、content、basisType、basisId、order | 问题及生成依据 |
| ResearchPlan | id、workspaceId、name、objective、ownerId、startAt、endAt、status | 调研计划 |
| PlanTarget | id、planId、sampleId、questionSetSnapshot、scheduledAt、duration、method、ownerId、participantIds、status | 最小执行单元；同一计划内样本唯一 |
| InterviewPack | id、planTargetId、companyId、version、generatedAt | 企业调研准备包 |
| InterviewQuestion | id、packId、category、content、basisType、basisId、order | 问题及生成依据 |
| ResearchRecord | id、planTargetId、companyId、interviewedAt、summary、status | 一次访谈记录；与计划对象一一对应 |
...
> 界面术语映射：`ResearchTask` 对外显示为“调研项目”；`ResearchSample` 对外显示为“调研对象”；`QuestionSet` 对外显示为“访谈提纲”；`PlanTarget` 对外显示为“企业调研安排”。`frozenAt` 是提纲定稿的内部审计字段，界面显示“定稿并用于计划”。
...
| IntelligenceItem | id、workspaceId、type、title、sourceUrl、publishedAt、capturedAt、summary、verificationStatus、companyIds、topicIds | 外部动态；当前原型直接保存企业和专题关联，后续可拆分为 IntelligenceLink |
...
`企业库筛选 → 用户加入调研对象 → 已选定 → 已计划 → 已预约 → 待执行 → 执行中 → 已完成/需复访/已取消`。企业库筛选不会落库创建候选对象。计划对象进入已完成时必须存在唯一调研记录；同一企业的再次调研通过新计划对象承载。
